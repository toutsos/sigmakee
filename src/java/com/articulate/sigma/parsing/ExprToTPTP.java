...
public class ExprToTPTP {
    ...
    /**
     * Thread-local snapshot of the KB relations set.  Set this to {@code kb.kbCache.relations}
     * before translating a batch of formulas so that {@link #shouldAddMention} can do a single
     * O(1) {@code HashSet.contains()} instead of the per-atom {@link KB#isRelationInAnyKB} call
     * (which iterates all KBs and acquires a try/catch on every atom).  Remove it in the finally
     * block after the batch to avoid cross-request contamination.
     *
     * <p>When {@code null} (the default), falls back to the original per-atom KB lookup so that
     * callers that don't set the snapshot continue to work correctly.</p>
     */
     ...
     * Produces the same result as {@link #getQlist(String)} for the same
     * formula.</p>
     *
